...
// Helper to pre-process photos (cover crop) exactly like IDCard.jsx does
...
const captureIDCard = async (frontEl, backEl) => {
  ...
  wrap.style.cssText = 'position:fixed;left:-99999px;top:0;width:700px;z-index:-999;pointer-events:none;background:#FFFFFF;display:flex;flex-direction:row;gap:20px;padding:20px;';
  document.body.appendChild(wrap);

  const cloneCard = (src, newId) => {
    const clone = src.cloneNode(true);
    clone.id = newId;
    clone.style.cssText = 'width:320px;height:480px;box-sizing:border-box;overflow:hidden;transform:none;position:relative;opacity:1;display:flex;flex-direction:column;background:#FFFFFF;border-radius:8px;font-family:Catamaran,sans-serif;';
...
const buildFormPage = (member) => {
  const logoSrc = window.location.origin + '/logo.png';
  const photoHTML = (member.photo_url || member.photo_base64)
    ? '<img src="' + (member.photo_url || member.photo_base64) + '" crossorigin="anonymous"/>'
  ...
  return [
    ...
    '<div class="id-box"><div class="id-label">உறுப்பினர் பதிவு எண்</div><div class="id-value">' + (member.member_id || '') + '</div></div>',
    '</div>',
    '</div>',
    '<hr class="hr"/>',
    ...
    '<div class="pledge-text">ஐயா,<br/><u>' + (member.full_name || '________________________') + '</u> ஆகிய நான் தென்னிந்திய வெல்டிங் தொழிலாளர் நலச்சங்கத்தின் <u>' + (member.district || '____________') + '</u> மாவட்டம் <u>' + (member.branch || '____________') + '</u> கிளைசங்கத்தில் உறுப்பினராக சேர்ந்து பணியாற்ற சம்மதிக்கின்றேன்.</div>',
...
export const bulkDownloadMembers = async (members, onProgress) => {
  ...
  // 1. Build All-Forms HTML file
  const allPages = members.map(m => buildFormPage(m)).join('\n');
  const formsFile = [
    '<!DOCTYPE html>',
    '<html lang="ta">',
    '<head>',
  ...
  // 2. Generate ID Card PNGs via hidden React render
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    // Render IDCard component for this member
